...
$(NAME): $(OBJS)
	...
	@printf "⠀⠀⠀$(PURPLE)⣦⣅⠢⠍⠉⠉⠈⢊⣙⡀⠂$(INDI)⠀⠀⠀⠀⠀⠰⠦⠥⡽⠋⠁⢐⠠⠂⡀⠁⠀⠁⠎⢣⠐⠎⡂⡀⠑⢄⣻⣛⠋⠑⣪⡗⠃⠀⠀⠀⠀⠐⠿⠟$(BRIGHT_RED)⠀⠹⣶⡀⠛⠿⣟⡀⢀⢟⣬⠀⡿$(GREEN)⡑⠁⠀⠃$(GOLD)⠀⠞⠋⠀⢀⡇⠱⠘⣧⠘⠌⢳⡀⠀⠈⠻⡄⣌⠠⠀⠀⠂⠙⡆⡄⠻⡄⢃⡆⡨$(DARK_BLUE)⠠⢭⣭⣾⣿⣿⠿⣿⣻⣿⣿⢷⣽⠃⠀⡄⢆$(RESET)\n"
	@printf "⠀$(PURPLE)⣠⡾⡿⠂⡰⠀$(RESET)⣤⣧⣄$(PURPLE)⠒⠀⠀⠥⠀$(BROWN)⠀⠒⢒⠀⠀⢤⡄⠐⠀⠀⡀⠀⠻⢿⣿⣷⣶$(INDI)⡱⢠⠠⣤⠔⢤⠚⢂⢢⡄⢀⡘⠃⠀⠀⠀⠀⠀⠀⠀$(BRIGHT_RED)⢠⣿⣄⠀⠀⠙⠀⠀⠈⠁⠈⡏⠇$(GREEN)⠈⠔⠀⠀⠀⠀⠀⠀⠀⠀$(GOLD)⣼⠝⡠⠃⠈⠆⠈⠀⠡⠀⠌⢦⣀⠙⢠⠀⡀⣶⣄⡐⠸⡀⢻⡀$(DARK_BLUE)⡜⠌⡳⣓⠽⡿⠛⠡⠒⠙⠿⢿⣿⣓⢁⣠⠺⣿⡆$(RESET)\n"
	@printf "⠀⣩⣵⣃⠀⠁⡴⣿⣟⣯$(PURPLE)⠰⡦⠐⠐⠶⠂$(BROWN)⢀⣨⣤⣶⠞⠁⠐⢶⢶⠇⡀⠀⠈⢙⠿⢫$(INDI)⢃⣾⡦⡉⠀⣈⠦⢊⠺⡷⠺⠁⠀⡐⠂⠀⠀⠀⠀$(BRIGHT_RED)⢠⣾⣿⣿⣦⣀⢀⡀⠀⠀⢀⠠⢱⠀⠀⠀⠀⠀$(GREEN)⡀$(GOLD)⠀⠀⠀⠀⠀⢉⣴⠃⢀⠀⠀⠀⠀⠠⣄⠐⢮⠛⢠⣌⠀⢣⡿⣽⣧⠀⢣⠘⣇⢡$(DARK_BLUE)⠐⢳⣽⡔⠡⢀⣠⣆⠀⠐⠚⢯⢛⣬⡯⠴⣿⡓$(RESET)\n"
	@printf "⠀⢉⣉⠛⠿⡆⣯⣉⡭⠍⢂⡔⠂⠀⠀⠁$(BROWN)⠻⣿⣿⠁⠀⢀⠘⡠⢜⣀⡾⠀⠀⠀⠀$(ORANGE)⢃⢸⡏⢩⣕⠊⠀⠉⠂⠀⠠⢂⡀⠀⠡⣓⡀⠀⠀$(BRIGHT_RED)⢀⣿⣿⣿⣟⢹⣿⠿⣪⣿⣿⣈⠤⠈⠁$(GREEN)⣤⣶⡿⠉⠩⣥⠀⠀⠀⠀$(GOLD)⠉⣠⢆⠀⡣⠍⠱⠷⣒⣬⡺⣬⣿⣥⣶⣶⣶⣿⣿⣿⠀⠈⠀$(DARK_BLUE)⠨⠈⠑⠒⠙⢏⠀⢹⣿⣿⣶⣒⠩⠑⢽⢩⠉⠀⠘⠃$(RESET)\n"
	@printf "⠀⠈⠛⠿⣦⢀⠐⠆⡠⢀⡒⠀$(YELLOW)⢲⡿⡿⣝⣕⣌⡳⠀$(BROWN)⠀⢠⠀⢛⡓⣉⡕⠐⡀$(ORANGE)⣴⣦⡌⢸⡱⠋⠈⠀⠀⠀⠀⠀⠀⢀⠃⠠⡒⠋⠀⠀⠀$(BRIGHT_RED)⣸⣿⣫⣶⡾$(PURPLE)⣦⡀⢰⡙⣣⠞⣫⠴⡲⠦$(GREEN)⠢⢹⣶⣧⣠⣿⡃⠀⠀$(GOLD)⡀⢼⡚⢇⡂⠉⣷⢿⣿⣿⡿⠁⢹⣿⣿⣿⣿⡿⠛⠉⠁⠀⠀$(DARK_BLUE)⠉⠀⠀⠀⠀⠀⠀⠀⠀⠙⢿⣽⣿⢆⢐⠄⡛⣁⢦⠀⠀$(RESET)\n"
	@printf "⠀⠀⠀⢀⣐⠮⠥⠄⣀⡠$(YELLOW)⡀⢶⡿⡇⠹⠜⠎⢫⡄⠁$(BROWN)⠀⡄⢩⣐⡉⠌⡈⠴$(ORANGE)⠀⠉⠉⠀⠘⣠⡄⠀⢀⠀⠀⠀⠀⠀⢸⡇⢀⡠⠄⠀⠀⠀$(BRIGHT_RED)⣿⠃$(PURPLE)⠈⠻⡧⠹⢯⠳⣣⠉⠊⡔⠣⠄⠙⢻⣦$(GREEN)⠛⠙⢿⡿⠀⠀$(GOLD)⠈⠣⢀⠑⠆⠙⠤⠀⠙⣿⣻⡵⣖⠤⢿⣿⣿⣿⡇⠀⠀⠀$(DARK_BLUE)⠁⠀⠀⢠⡀⠀⠀⠀⠀⠀⠀$(GOLD)⢀⣶⣦⣰⡀⠀⠀⠀$(DARK_BLUE)⠋⠈⠀⠀$(RESET)\n"
	@printf "⠀⠀⣰⣭⣛⡛⣛⣫⠍$(YELLOW)⠀⠧⠀⠘⠈⣀⡄⠀⠀⠀⠀$(BROWN)⠐⠃⠈⠻⠏⠀⠀⣱$(ORANGE)⠀⠀⠀⠀⢀⠑⠠⣦⣨⣬⠀⠀⠀⠀⣼⣇⡉⡸⠁⠀⠀⠀$(BRIGHT_RED)⢹⠀⡀$(PURPLE)⠔⢊⠵⠉⠂⠀⠠⢶⣶⠶⣦⣄⠀⠺$(GREEN)⢕⣰⠂⠚⡟⢧⣉⠐⡒⢄$(GOLD)⠠⠁⣀⠁⠀⠀⠁⠛⢼⣫⣭⣿⣿⢟⣾⡦⠀⠀$(DARK_BLUE)⢐⠎⢗⠈⢰⠀⠀⠀⠀⠀⠀$(GOLD)⠈⣿⣿⣷⢵⣕⣔⣀⡀⠀⣠⡇\n"
	@printf "⠀⠀$(RESET)⢹⣛⣙⠁$(YELLOW)⠨⠡⠄⠀⠀⠀⠀⣌⣿⣷⣾⠟⣱⡇$(BROWN)⠀⠀⠀⠀⠀⠀⡰⠁$(ORANGE)⠀⠀⠀⠀⠈⠳⠦⠙⠿⠮⠉⠀⠀⠀⣉⠅⠀⠁⠀⠀⠀⠀$(BRIGHT_RED)⠘$(PURPLE)⠄⠘⠀⢃⠒⡡⠄⢤⠩⢍⠈⠓⢡⠈⠓⡀⠀$(GREEN)⢹⡀⠈⢀⠃⢚⡓⠦⠈⢂⠦⣐⠦⣄⠤⢀⠀⠀⠀$(GOLD)⠋⠉⢠⡾⠃⣠⡧⠀$(DARK_BLUE)⢼⠎⠈⢿⣆⠀⠀⠀⠀⠀⠀$(GOLD)⣠⠿⠻⠟⣸⡞⣿⣿⣿⣿⣷⣽\n"
