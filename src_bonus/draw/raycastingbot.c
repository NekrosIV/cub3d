...
/*   Updated: 2024/10/14 17:38:58 by kasingh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_enemy_column(t_game *game, char *data, t_enemy *bot, t_utils *u)
{
		...
		if (game->profondeur[u->startx] > u->distance)
		{
			...
			while (u->i < u->endy && u->i < WINY && u->i >= 0)
			{
				if (u->x_img < 64 && u->y_img < 64
					&& *((int *)game->botext[bot->action][bot->frame].data
					+ (int)u->x_img + (int)u->y_img
					* 64)
					!= *((int *)game->botext[bot->action][bot->frame].data))
				{
					*((int *)data + u->i * WINX
