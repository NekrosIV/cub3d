...
void	draw_loop(t_game *game, int line_size, float offset_x, float offset_y)
{
	...
	while (game->map_y < game->map_max_y)
	{
		...
		while (game->map_x < line_size)
		{
			...
			if (screen_x + TILE_SIZE > 0 && screen_x < (M / 4) && screen_y
				+ TILE_SIZE > 0 && screen_y < (M / 4))
			{
...
{
	if (playerpos * TILE_SIZE < (M / 4) / 2.0f)
		return (0.0f);
	else if (playerpos * TILE_SIZE > max * TILE_SIZE - (M / 4) / 2.0f)
		return ((M / 4) - max * TILE_SIZE);
	else
		return (((M / 4) / 2.0f) - playerpos * TILE_SIZE);
}
...
void	mini_draw_map(t_game *game, t_texture *textures)
{
	...
	offset_x = 0.0f;
	offset_y = 0.0f;
	if (game->map_max_x * TILE_SIZE > (M / 4))
		offset_x = find_offset(game, game->player.posx, game->map_max_x);
	if (game->map_max_y * TILE_SIZE > (M / 4))
...
void	draw_enemies_on_minimap(t_game *game, t_texture *textures, t_utils *u)
{
	...
	while (g < game->bot_nb)
	{
		...
		else if (screen_x > 0 && screen_x < (M / 4) && screen_y > 0
			&& screen_y < (M / 4) && game->ennemy[g].animating == 0)
